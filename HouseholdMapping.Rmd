...
House_Mapping <- House_Mapping %>%
  add_column(`Settlement Locality` = 
               case_when(
                 (House_Mapping$lga_name == "Alimosho")  ~ House_Mapping$settlement_locality_1
...
House_Mapping_Joiner <- House_Mapping %>%
  dplyr::select(record_id, lga_name, mapping_id, data_entry_personnel, date_of_mapping, Enumerator, Status)
...
Tot_No_tot_babies_lost_womb_Joined <- 
  left_join(Tot_No_tot_babies_lost_womb, Stillbirth, by = c('record_id'='record_id', 'about_household'='stillbirth_household_number'))
...
Tot_No_tot_women_delivered_died
Tot_No_tot_babies_lost_womb
Tot_No_tot_num_girl_women_cur_preg
...
for (i in 1:15) {
  trend_df[i, "Cumulative Mapping (expected)"] <- trunc(i * 1067)
}
...
trend_df['Daily Mapping (expected)'] <- 1067
...
ggplot(trend_df, aes(x = `Mapping Day`)) +
  ...
  scale_y_continuous(breaks= seq(700, 22400, by=700))
